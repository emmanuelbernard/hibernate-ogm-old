...
public class OgmTableGenerator implements PersistentIdentifierGenerator, Configurable {
	...
	public Serializable doWorkInCurrentTransaction(SessionImplementor session) {
		...
		final Object segmentColumnValue = nullSafeSet( segmentGridType, segmentValue,segmentColumnName, session, tuple );
		...
		do {
			...
			final Object dbValue = identifierCache.withFlags(Flag.SKIP_LOCKING).get( key );
			if ( dbValue == null ) {
				//if not there, insert initial value
				value.initialize( initialValue );
				Object idValue = nullSafeSet( identifierGridType, value.makeValue(), valueColumnName, session, tuple );
				final Object oldValue = identifierCache.putIfAbsent( key, idValue );
				//check in case somebody has inserted it behind our chack
				if (oldValue != null) {
				...
				//DB value has to be a long
				value.initialize( ( (Long) dbValue ).longValue() );
			}
			...
			final Object newValue = nullSafeSet( identifierGridType, updateValue.makeValue(), valueColumnName, session, tuple );
			final Object oldValue = nullSafeSet( identifierGridType, value.makeValue(), valueColumnName, session, tuple );
			done = identifierCache.replace( key, oldValue, newValue );
		}
		while ( !done );
